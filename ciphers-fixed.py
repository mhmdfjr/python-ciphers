...
def playfair_encrypt(plain_text, key):
    ...
    cipher_text = ""
    for a, b in digraphs:
        a_row, a_col = np.where(matrix == a)[0][0], np.where(matrix == a)[1][0]
        b_row, b_col = np.where(matrix == b)[0][0], np.where(matrix == b)[1][0]
        if a_row == b_row:
...
def hill_encrypt(plain_text, key_matrix):
    ...
    if len(text_vector) % n != 0:
        text_vector.extend([0] * (n - (len(text_vector) % n)))  # Padding dengan nilai 0 (a)
    
    encrypted_vector = []
    for i in range(0, len(text_vector), n):
        vector = text_vector[i:i+n]
        result = np.dot(key_matrix, vector) % 26
        encrypted_vector.extend(result)
...
def hill_decrypt(cipher_text, key_matrix):
    ...
    det = int(np.round(np.linalg.det(key_matrix)))  # Determinan dari key_matrix
    det_inv = mod_inv(det, 26)  # Cari invers modulo 26 dari determinan
    if det_inv is None:
        raise ValueError("Key matrix tidak bisa di-inverse dengan mod 26")
    ...
    for i in range(0, len(cipher_vector), n):
        vector = cipher_vector[i:i+n]
        result = np.dot(inverse_key_matrix, vector) % 26
        decrypted_vector.extend(result)
